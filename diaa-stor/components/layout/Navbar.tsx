...
import { Sun, Moon, Menu, X, Search, ShoppingCart, Globe, ChevronDown } from 'lucide-react'
...
export default function Navbar() {
  ...
  const textClass = (!isHome || scrolled) ? 'text-slate-700 dark:text-slate-200' : 'text-white'
  const logoText = (!isHome || scrolled) ? '' : 'opacity-0 sm:opacity-100'
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <Link href="/" className="flex items-center gap-2.5 flex-shrink-0">
              <div className="relative w-10 h-10 flex-shrink-0">
                <Image src="/logo.png" alt="Diaa Store" fill className="object-contain" priority />
              </div>
            ...
            <div className="flex items-center gap-1">
              {/* Search */}
              <button onClick={() => setSearchOpen(s => !s)} className={`btn-ghost p-2.5 rounded-xl ${textClass}`} aria-label="Rechercher">
                <Search size={18} />
              </button>

              {/* Cart */}
              <Link href="/cart" className={`relative btn-ghost p-2.5 rounded-xl ${textClass}`} aria-label="Panier">
                  ...
                  <div className="absolute top-full right-0 mt-2 w-40 card py-1 z-50 animate-fade-in">
                    {langs.map(l => (
                      <button key={l.code} onClick={() => switchLang(l.code)}
                        className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors ${router.locale === l.code ? 'font-bold text-navy-700 dark:text-gold-400' : 'text-slate-600 dark:text-slate-300'}`}>
              ...
              {mounted && (
                <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
                  className={`btn-ghost p-2.5 rounded-xl ${textClass}`} aria-label="Thème">
              ...
              <button onClick={() => setMobileOpen(s => !s)}
                className={`btn-ghost p-2.5 rounded-xl md:hidden ${textClass}`} aria-label="Menu">
            ...
            <div className="pb-3 animate-slide-up">
              <form onSubmit={handleSearch} className="flex gap-2">
                <input autoFocus type="text" value={searchQ} onChange={e => setSearchQ(e.target.value)}
                  placeholder="Rechercher un produit..." className="input-field flex-1" />
                <button type="submit" className="btn-primary px-5 py-2.5"><Search size={16} /></button>
                <button type="button" onClick={() => setSearchOpen(false)} className="btn-ghost px-3 rounded-xl border border-slate-200 dark:border-slate-600"><X size={16} /></button>
              </form>
            </div>
          )}
        </div>
          ...
          <div className="md:hidden bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 animate-slide-up">
            <div className="px-4 py-3 space-y-1">
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex gap-2">
                {langs.map(l => (
                  <button key={l.code} onClick={() => switchLang(l.code)}
                    className={`flex-1 py-2 rounded-xl text-xs font-bold transition-colors ${router.locale === l.code ? 'bg-navy-700 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>
